...
```sh
python3 test/wolfi/playwright-vscode/prepare.py \
  --lock config/wolfi-dev.lock.json \
  --base-image wolfi-bootstrap-test:20260905-arm64 \
...
After building an isolated dev profile with Playwright and VS Code enabled and
`docker.socket: false`:

```sh
python3 test/wolfi/playwright-vscode/run.py \
  --config .tmp/playwright-dev.yaml \
  --lock .tmp/playwright-dev.lock.json \
...
archive. Both client and target containers use `--network=none`; only the external
...
Success requires an actual remote Dev Containers extension host with the correct
Wolfi user, VS Code version and browser manifest; a real task exit of zero; the
