...
def main():
    ...
    if config.get("docker", {}).get("socket", False):
        raise ValueError("The browser acceptance profile requires docker.socket: false")
    ...
    if image["Config"]["Labels"].get("devcontainers.wolfi.lock.sha256") != lock_hash:
    ...
    shutil.copytree(ROOT / "src/wolfi/components/playwright/fixture", workspace)
    ...
    (workspace / ".vscode/tasks.json").write_text(json.dumps({"version": "2.0.0", "tasks": [{
        "label": "Wolfi: Playwright visual acceptance", "type": "process", "command": "/bin/bash",
    ...
    dc = {"name": f"Wolfi Playwright editor acceptance {identifier}",
    ...
    try:
        ...
        if target_details["Config"].get("Labels", {}).get("devcontainers.wolfi.lock.sha256") != lock_hash:
            raise AssertionError("Dev Container differs from the selected locked image")
        if any("docker.sock" in mount.get("Destination", "") or "docker-host.sock" in mount.get("Destination", "")
               for mount in target_details.get("Mounts", [])):
            raise AssertionError("Playwright target must not mount a Docker socket")
        ...
        run(["docker", "exec", target, "/bin/sh", "-c",
             "test ! -S /var/run/docker.sock && test ! -S /var/run/docker-host.sock && ! command -v dockerd"])
