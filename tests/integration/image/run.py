...
def run_script(image, platform, script, env=None):
    ...
    marker = "TOOLBOX_JOB_SCRIPT_COMPLETED"
    ...
    try:
        created = json.loads(run(["docker", "inspect", container], capture_output=True, text=True).stdout)[0]
        assert created["HostConfig"]["Privileged"] is False, "test job unexpectedly uses privileged mode"
        assert created["HostConfig"]["NetworkMode"] == "none", "test job unexpectedly has external networking"
        assert not created["HostConfig"].get("Binds"), "ordinary profile test unexpectedly mounts host paths"
...
def main():
    ...
    assert labels.get("devcontainer-blueprints.lock.sha256") == hashlib.sha256(raw).hexdigest(), "image lock label differs"
    assert labels.get("devcontainer-blueprints.config.sha256") == lock["source"]["fileSha256"], "image YAML label differs"
    assert labels.get("devcontainer-blueprints.settings.sha256") == lock["source"]["settingsFileSha256"], "image settings label differs"
    assert labels.get("devcontainer-blueprints.profile") == config["profile"], "image profile label differs"
    ...
    for key in ("java", "maven", "node", "npm", "kubectl", "yq", "helm", "oras", "mongosh"):
    ...
    catalog = json.loads((ROOT / "src/components/utilities/catalog.json").read_text())
    ...
    run_script(image, platform, (HERE / "runtime.sh").read_text() + "\n" + (ROOT / "tests/integration/components/utilities.sh").read_text(), env)
    ...
    for key, executable in {"kubectl": "kubectl", "rust": "rustc", "helm": "helm", "oras": "oras",
                            "mongosh": "mongosh", "mongodbDatabaseTools": "mongodump"}.items():
        if key not in tools:
    ...
    if "playwright" not in config:
        ...
        command = ["python3", ROOT / "tests/integration/components/playwright_image.py", "--lock", args.lock]
        if args.quick:
            command += ["--quick"]
        run(command)
    if "kaniko" in config:
        ...
        if not args.quick:
            ...
            run([ROOT / "tests/integration/components/kaniko_build.py", "--image", image, "--platform", platform,
                 "--base-archive", ROOT / artifact["artifactDirectory"] / artifact["file"],
                 "--base-sha256", artifact["sha256"]])
    else:
        run([*base, image, "-c", "test ! -e /kaniko"])
    if config["profile"] == "build":
        socket_path = Path("/var/run/docker.sock")
        assert socket_path.exists(), "build-profile socket test requires the explicit host Docker socket"
        command = ["docker", "run", "--rm", "--platform", platform, "--network=none",
                   "--mount", f"type=bind,source={socket_path},target=/var/run/docker.sock",
                   "--entrypoint", "/bin/bash", image, "-c",
                   "set -Eeuo pipefail; docker version --format '{{.Client.Version}} {{.Server.Version}}'; "
                   "docker info --format '{{.ServerVersion}}'; ! command -v dockerd; ! command -v containerd"]
        run(command)
    if "vscode" in config:
        command = [HERE / "vscode.sh", "--lock", args.lock]
        if args.quick:
            command += ["--quick"]
        run(command)
    else:
    ...
    if "rust-analyzer" in tools.get("rust", {}).get("components", []):
        run_script(image, platform, (HERE / "rust_lsp.sh").read_text())
    if "helm" in tools:
        container = run(["docker", "create", "--platform", platform, "--network=none", "--entrypoint", "/bin/bash", image,
                        ...
                        capture_output=True, text=True).stdout.strip()
        try:
            run(["docker", "cp", ROOT / "tests/fixtures/helm", f"{container}:/tmp/helm-smoke"])
            ...
            run(["docker", "rm", "-f", container], stdout=subprocess.DEVNULL)
    if config.get("devcontainer", False):
        command = [HERE / "devcontainer.sh", "--lock", args.lock]
        if args.quick:
