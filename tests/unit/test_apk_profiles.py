...
class ApkProfileTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        cls.mapping = load_package_mapping(SCRIPTS / "package-roots.json")
        cls.profiles = {
            name: json.loads(subprocess.check_output([
                "node", str(ROOT / "src/config/config.mjs"), "print-json",
                str(ROOT / f"config/{name}.yaml"),
            ], text=True))
            for name in ("dev", "build", "kaniko")
        }
    ...
    def test_build_has_docker_tools_without_dev_runtime_roots(self):
        roots, modules = self.roots(self.profiles["build"])
        names = {root["name"] for root in roots}
        self.assertEqual(modules, ["base", "docker", "build", "utilities", "playwright"])
        self.assertTrue({"bash", "grep", "git", "jq", "gnutar", "gzip"} <= names)
        self.assertTrue({"docker-cli", "docker-cli-buildx", "docker-compose"} <= names)
        self.assertTrue({"socat", "sudo", "shadow"}.isdisjoint(names))
    ...
    def test_profiles_share_toolchain_package_roots(self):
        ...
        self.assertEqual(roots["build"], roots["dev"])
        self.assertEqual(roots["kaniko"], roots["dev"])
    ...
    def test_minimal_profile_has_only_base_roots(self):
        config = copy.deepcopy(self.profiles["build"])
        config["build"] = {}
        config["utilities"] = {}
    ...
    def test_one_closure_records_exact_selected_root_constraints(self):
        ...
        result = resolver.build_package_set_records(
            ...
            artifact_directory="artifacts/build/linux-amd64/apk",
        )
    ...
    def test_offline_test_retains_base_world_and_installed_database(self):
        installer = load_test_module(
            ROOT / "tests/integration/supply/apk_offline_install.py"
        )
