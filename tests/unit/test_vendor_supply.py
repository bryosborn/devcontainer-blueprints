...
VENDOR_SOURCE = ROOT / "src/supply/vendor"


def load(name, filename):
    spec = importlib.util.spec_from_file_location(name, VENDOR_SOURCE / filename)
...
class VendorTests(unittest.TestCase):
    def test_rust_install_does_not_require_unselected_rustfmt_or_clippy(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            source = (ROOT / "src/components/rust/install.sh").read_text()
    ...
    def test_empty_selection_does_not_resolve_or_fetch(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            lock = root / "lock.json"
            lock.write_text(json.dumps({"schemaVersion": 3, "resolved": {}}))
...
""")
            ...
            for components in ("rust-analyzer", ""):
                subprocess.run([str(ROOT / "src/components/rust/prefetch.sh"),
