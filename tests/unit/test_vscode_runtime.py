...
COMPONENT_TEST = ROOT / "tests/integration/image/extension_components.mjs"
...
class VSCodeRuntimeTests(unittest.TestCase):
    ...
    def test_schema3_root_image_defaults_to_install_and_quick_skips_it(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            lock = root / "lock.json"
            lock.write_text(json.dumps({"schemaVersion": 3,
                                        "source": {"fileSha256": "b" * 64, "settingsFileSha256": "c" * 64},
                                        "image": {"reference": "test/vscode:root", "platform": "linux/amd64"},
...
if args[:2]==['image','inspect']:
    if '--format' in args:
        ...
        if '.Os' in expression: print('linux/amd64')
        elif 'config.sha256' in expression: print('b'*64)
        elif 'settings.sha256' in expression: print('c'*64)
        else: print(os.environ['WOLFI_TEST_LOCK_SHA'])
    else: print('[]')
elif args[0]=='create':
...
''')
            ...
            for flags, install in (([], "true"), (["--quick"], "false")):
                subprocess.run([str(ROOT / "tests/integration/image/vscode.sh"), "--lock", str(lock), *flags],
            ...
            for failure in ("missing-marker", "failed-state", "oom-state"):
                with self.subTest(failure=failure):
                    result = subprocess.run([str(ROOT / "tests/integration/image/vscode.sh"), "--lock", str(lock), "--quick"],
