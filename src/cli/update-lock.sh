...
usage() {
  cat <<'EOF'
Internal profile lock resolver
...
This is the only operation that resolves mutable selectors. It refreshes
...
WORKSPACE="$(mktemp -d "${REPO_ROOT}/.tmp/image-lock.XXXXXXXX")"
cleanup() {
...
node "${REPO_ROOT}/src/config/config.mjs" print-json "${CONFIG_FILE}" > "${CONFIG_JSON}"
CONFIG_HASH="$(node "${REPO_ROOT}/src/config/config.mjs" hash "${CONFIG_FILE}")"
...
node "${REPO_ROOT}/src/config/lock.mjs" \
  --config "${CONFIG_FILE}" \
  --lock "${BASE_LOCK}" \
  --base-only
...
"${REPO_ROOT}/src/supply/apk/prefetch.sh" \
...
if jq -e 'has("vscode") or has("kaniko") or has("playwright") or (.utilities | has("kubectl")) or (.build | has("rust"))' "${CONFIG_JSON}" >/dev/null; then
  python3 "${REPO_ROOT}/src/supply/vendor/resolve-vendor.py" \
    --config-json "${CONFIG_JSON}" \
...
node "${REPO_ROOT}/src/config/lock.mjs" "${UPDATE_ARGS[@]}"
node "${REPO_ROOT}/src/config/config.mjs" verify-lock "${CONFIG_FILE}" "${LOCK_FILE}" >/dev/null

echo "Profile lock update complete:"
