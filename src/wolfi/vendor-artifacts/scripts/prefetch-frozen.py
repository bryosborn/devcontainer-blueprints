...
def fetch(url: object, destination: Path, expected: str) -> None:
...
def main() -> None:
    ...
    fetch(server.get("url"), server_path, validate_hash(server.get("sha256"), "VS Code SHA256"))
    ...
    if not archive_path.is_file() or sha256(archive_path) != expected_archive_hash:
        subprocess.run(
            [
                str(repo_root / "src/base-vscode/scripts/package-extensions.sh"),
                "--lock",
