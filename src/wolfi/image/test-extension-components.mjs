...
function commandAvailable(command) {
  return spawnSync("sh", ["-c", 'command -v "$1" >/dev/null 2>&1', "probe", command]).status === 0;
}

function selected(id, commands = []) {
  const present = readdirSync(extensionsRoot, { withFileTypes: true }).some(
    (entry) => entry.isDirectory() && entry.name.toLowerCase().startsWith(`${id.toLowerCase()}-`),
  );
  if (!present) return false;
  const missing = commands.filter((command) => !commandAvailable(command));
  if (missing.length) {
    console.log(`SKIP ${id}: optional runtime unavailable (${missing.join(", ")})`);
    return false;
  }
  return true;
}

const pythonCommand = ["python3.13", "python3.12", "python3"].find(commandAvailable);
...
try {
  if (selected("ms-python.debugpy", [pythonCommand || "python3"])) {
  ...
  if (selected("ms-vscode.cpptools", [])) {
  ...
  if (selected("rust-lang.rust-analyzer", ["rustc", "rustup"])) {
  ...
  if (selected("redhat.vscode-yaml", [])) {
  ...
  if (selected("redhat.vscode-xml", ["java"])) {
  ...
  if (selected("ms-azuretools.vscode-containers", [])) {
