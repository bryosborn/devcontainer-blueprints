...
SPEC = importlib.util.spec_from_file_location("playwright_supply", ROOT / "src/wolfi/vendor-artifacts/scripts/playwright_supply.py")
...
def main():
    ...
    if f"{details['Os']}/{details['Architecture']}" != platform or details["Config"].get("Labels", {}).get("devcontainers.wolfi.lock.sha256") != digest:
    ...
    for identity, uid, gid, update in identities:
        for mode in ("headless-shell", "full-chromium", "headed"):
            ...
            script = ["set -Eeuo pipefail", "cd /tmp/wolfi-playwright-fixture",
                      "tar -xzof /tmp/wolfi-playwright-runner.tar.gz", "rm /tmp/wolfi-playwright-runner.tar.gz",
            ...
            script += [f"chown -R {uid}:{gid} /tmp/wolfi-playwright-fixture",
            ...
            command = f"set -eu; cd /tmp/wolfi-playwright-fixture; test \"$(id -u)\" = {uid}; test \"$(id -g)\" = {gid}; {executable}"
            script.append(command if identity == "root" else f"su -s /bin/sh {shlex.quote(identity)} -c {shlex.quote(command)}")
            script.append("printf '%s\\n' WOLFI_PLAYWRIGHT_CONTAINER_COMPLETED")
            ...
            try:
                run(["docker", "cp", HERE / "fixture", f"{container}:/tmp/wolfi-playwright-fixture"])
                run(["docker", "cp", runner, f"{container}:/tmp/wolfi-playwright-runner.tar.gz"])
                ...
                copied = subprocess.run(["docker", "cp", f"{container}:/tmp/wolfi-playwright-fixture/results/.", str(output)], capture_output=True, text=True)
                ...
                if "WOLFI_PLAYWRIGHT_FIXTURE_PASS" not in logs.stdout or logs.stdout.splitlines()[-1:] != ["WOLFI_PLAYWRIGHT_CONTAINER_COMPLETED"]:
