...
def main():
    ...
    run([ROOT / "src/cli/prefetch.sh", "--config", args.config, "--lock", args.lock, "--offline"])
    ...
    workspace = Path(tempfile.mkdtemp(prefix="image-build-", dir=ROOT / ".tmp"))
    try:
        ...
        options = [
            "--network=none",
            "--build-context", f"wolfi_apks={ROOT / selected['artifactDirectory']}",
            "--build-context", f"wolfi_vendor={vendor}",
            "--label", f"devcontainer-blueprints.lock.sha256={hashlib.sha256(lock_bytes).hexdigest()}",
            "--label", f"devcontainer-blueprints.config.sha256={lock['source']['fileSha256']}",
            "--label", f"devcontainer-blueprints.settings.sha256={lock['source']['settingsFileSha256']}",
            "--label", f"devcontainer-blueprints.profile={config['profile']}",
        ]
        ...
        run(["bash", "-c", 'source "$1/src/cli/common.sh"; wolfi_verify_image_lock "$2" "$3"',
