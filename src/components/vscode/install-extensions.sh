...
resolve_vsix_path() {
  local locked_path="$1" extension_id="$2" extension_version="$3"
  local candidate

  if [[ -n "${PAYLOAD_DIR}" ]]; then
    [[ "${extension_id}" != */* && "${extension_version}" != */* \
      && "${extension_version}" != . && "${extension_version}" != .. ]] || return 1
    candidate="${PAYLOAD_DIR}/server/${extension_id,,}/${extension_version}/$(basename "${locked_path}")"
  ...
  if [[ "${locked_path}" != /* && -f "${LOCK_DIR}/${locked_path}" ]]; then
    printf '%s\n' "${LOCK_DIR}/${locked_path}"
    return 0
  fi
...
for ext_id in "${EXT_IDS[@]}"; do
  ...
  if [[ -z "${vsix_rel}" || -z "${expected_sha}" || -z "${version}" ]]; then
    echo "ERROR: missing lockfile VSIX metadata for ${ext_id}" >&2
    exit 1
  fi

  if ! vsix_path="$(resolve_vsix_path "${vsix_rel}" "${ext_id}" "${version}")"; then
