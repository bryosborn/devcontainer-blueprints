...
import { loadImageSettings } from './settings.mjs';
...
const schema={
  $schema:'http://json-schema.org/draft-07/schema#', title:'Toolbox image profile',
  ...object({
    schemaVersion:{const:3},
    profile:{enum:settings.profiles},
    image:object({platform:{enum:['linux/amd64','linux/arm64']}},['platform']),
  ...
  },['schemaVersion','profile','image','wolfi'])
};
const target=new URL('../../config/image.schema.json',import.meta.url);
const text=JSON.stringify(schema,null,2)+'\n';
if(process.argv.includes('--check')) {
 if(fs.readFileSync(target,'utf8')!==text) throw Error('Editor schema is stale; run node src/config/schema.mjs');
