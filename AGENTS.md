...
CI is root without Docker/VS Code; dev adds named identity, VS Code and Docker
socket access. Both defaults select all shared tools and Playwright except
ClamAV. Kaniko is enabled only in CI and runs in a disposable packaging job.
...
- `docs/cve-report.md`: one checked-in scan summary for both complete defaults.
...
- 2026-09-05 - Decision: Kaniko's executor runs in a dedicated disposable GitLab job; consuming projects own their runner, registry, credential, and job configuration.
...
- 2026-09-05 - Finding: Trivy 0.74 filters two embedded Chainguard mongosh records while retaining the installed APK, but the tested database has no mongosh advisory entry. Complete package inventory alone does not prove advisory coverage; preserve the limitation in the CVE report.
...
- 2026-09-05 - Decision: Optional Playwright locks matched Chromium and headless shell, with native Wolfi prerequisites and focused fonts; no FFmpeg/video. Both complete defaults enable it.
...
- 2026-09-05 - Finding: The former Ubuntu Node artifact and Wolfi both expose Node, npm, npx, and Corepack. Node remains 24.20.0; Wolfi independently locks newer npm/Corepack packages, and runtime tests assert npx with the selected npm line.
